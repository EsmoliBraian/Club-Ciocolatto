...
export function BirthdayBanner({ favoriteDrink }: { favoriteDrink?: string | null }) {
  const [pending, startTransition] = useTransition();
  const drink = favoriteDrink ?? "café";
      ...
      if (result.success) {
        toast.success("¡Feliz cumpleaños! 🎂", {
          description: result.redemptionCode
            ? `Tu ${result.drink} gratis: código ${result.redemptionCode}. +${result.pointsAwarded} puntos.`
